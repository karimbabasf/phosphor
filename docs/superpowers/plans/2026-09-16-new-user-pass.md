...
## Stream PREFLIGHT (branch feat/preflight, wave C, after SEND is on main)

Karim, 2026-09-16 late: "implement a tracker that tracks the current gas on chain on arbitrum before sending out a 1click api call so that we dont result in a loss like we did due to a gas error. simulation should be a huge part but not in front of the user, in the backend. with it being verifiable in the transaction folded up if the user would like to see, but not as text, as a nice diagram maybe or just simple and ui friendly."

The loss (vault: Claude/Projects/phosphor-2026-09-15-audit-D-double-spend.md, line 126 onward): 1Click's HyperCore leg withdraws USDC to a forwarding wallet on Arbitrum and its relayer sweeps it into Circle CCTP with a hard-coded 300,000 gas limit; an Arbitrum L1 data surge put 155,024 gas of L1 data into that sweep and it ran out of gas, twice, unrefunded.

Owner of: src/preflight/* (new), src/rails/intents-spend.ts (the hook before submit-intent), src/rails/hypercore-deposit.ts and intents-pay.ts (call the hook, carry the result), src/types.ts `Proposal.preflight`, src/proposals/execute.ts (the held state and retry loop), ui/screens/checks.js (new renderer) and its css, ui/screens/receipt.js and the send card's checks slot, docs/security-model.md "Preflight", tests.

### Task F1: the module
`src/preflight/arbitrum.ts`: read, through `reader('arb')` (viem): the L2 base fee (`getBlock` baseFeePerGas), and the ArbGasInfo precompile `0x000000000000000000000000000000000000006C` `getPricesInWei()` (returns six uint256: perL2Tx, perL1CalldataByte, perStorageAllocation, perArbGasBase, perArbGasCongestion, perArbGasTotal) plus `getL1BaseFeeEstimate()`. Compute `sweepGasUnits = 145_000 + l1DataUnits` where `l1DataUnits = ceil(perL1CalldataByte * SWEEP_CALLDATA_BYTES / perArbGasTotal)` with `SWEEP_CALLDATA_BYTES = 420` (the observed CCTP `depositForBurn` sweep) and the vendor limit `VENDOR_SWEEP_GAS_LIMIT = 300_000`. Verdict: `ok` under 240,000, `elevated` 240,000 to 300,000, `blocked` above. Keep a 60-sample in-memory history (one per minute while any preflight ran in the last hour) so the receipt can draw a sparkline and the verdict can say "3x the hourly average".
`src/preflight/index.ts` exports `runPreflight(kind, draft, quote, deps): Promise<Preflight>` with `Preflight = { at, checks: PreflightCheck[], verdict: 'ok' | 'hold' | 'fail', holdReason?: string }` and `PreflightCheck = { id: 'gas' | 'coverage' | 'venue' | 'balance' | 'deadline', label, state: 'ok' | 'warn' | 'fail', value: string, detail: string, series?: number[] }`. Checks: gas (Arbitrum for hl_deposit and any payout landing on Arbitrum; Ethereum and Base payouts read their base fee against an hourly average with the same shape, verdict warn above 2x, fail above 4x), coverage (the quote's fee versus our own estimate of the payout cost: gas price times 65,000 units for a token, 21,000 for the chain's coin, converted through the ledger's price, warn under 1.5x cover, fail under 1.0x), venue (a dry quote answered within 10 s and `/v0/status` reachable), balance (verifier balance covers the amount), deadline (quote deadline at least 3 minutes away).
Tests with injected readers and fetchImpl: the 09-15 surge numbers (perL1CalldataByte high enough that l1DataUnits reaches 155,024) yield `blocked` and verdict `hold`; normal numbers yield `ok`; coverage under 1.0x fails; series keeps 60 samples.

### Task F2: the hook
In `spendFromIntents` (intents-spend.ts) before `generate-intent`: `runPreflight`; on `hold` the rail returns `{ status: 'held', preflight }` without signing, execute.ts keeps the row `approved` with `heldSince` and retries every 30 s for up to 15 minutes, then marks `failed` with the hold reason; each retry appends the new preflight. Nothing is ever signed while held. The card shows "Waiting for Arbitrum gas to settle (2 min)" from the hold reason, no question to the user. Tests: a held rail signs nothing (signer fake counts zero calls), the retry loop runs the preflight again and executes on `ok`, the 15 minute cap fails cleanly with an audit line `execution_held_expired`.

### Task F3: the checks diagram
`ui/screens/checks.js`: a folded section "Checks" under the receipt and inside the send card (closed by default, a chevron): a vertical rail of five nodes joined by a hairline, each node a 10 px dot (green ok, `--warn` warn, `--down` fail) with the label in Sora 13 px and the value in Geist Mono 13 px on the right, the detail in `--text-2` 12 px under it, and for the gas node a 120 by 28 px sparkline of the series with the vendor limit as a dashed line. Plain English labels: "Arbitrum gas", "Fee covers the payout", "Venue answering", "Balance", "Quote still valid". Tests in the receipt-ui style: five nodes render, states map to classes, the sparkline draws when a series exists, the fold toggles.

Deliverable: `reports/preflight.md` (30 lines) with the module API, the 09-15 replay numbers, and a PNG of the checks fold open.
