...
## Track T7a: the agent channel and the shell (audit A)

A1 (High): the driver child loads Claude Code auto-memory from a predictable path under the user's home, so a file any local process can write becomes instructions in the session that moves money. Fix: `CLAUDE_CONFIG_DIR` in `childEnv` pointing at a directory under the app's data dir, refuse to spawn when its memory folder is non-empty, and `tests/lockdown.test.ts` reads `memory_paths` from the init event. A2 (Medium): the shell identifies its backend by a fixed header; a boot nonce goes down the stdin pipe beside the window token, the backend echoes it in the `x-phosphor` value, the shell compares, the respawn path gets the pre-check, and the exit check runs before the liveness check. A4 (Medium): roster seats reserved for sessions the app minted; an outside hello needs a per-boot secret carried through `childEnv`. A5 to A8 (Low) where under thirty minutes. A3: the token section of `docs/security-model.md` rewritten to the model as built. Branch `wt/channel`.

## Track T7b: custody, policy and execution (audit B)

B1 (Critical): the receive address is served from an unauthenticated header; serve addresses derived from the decrypted payload while open, verify the wrap's tag over the header while locked, report `tampered` rather than `wrong_password`. B2 (High), backend half: a ceiling on `usdField` and a refusal for a patch that raises the click threshold past the per-transaction cap, multiplies a cap by more than ten, or shortens the allowlist; `mergePatch` never drops entries silently; the basic view renders the policy diff. B5 (High): the runner charges realised losses from the venue's own figure. B6 (High): the instance lock is taken by temp-file-and-rename with a compare-and-delete on the stale path. B7 (High): the transaction hash is persisted and audited the moment the rail returns, the balance later. B9 (Medium): the 24 hour budget charges legs that produced a hash and a partial send lands `needs_reconciliation`. B11 (Medium): one in-flight singleton for every password check, and the backoff re-checked after derivation. Lows B12, B14 to B17 where cheap. Branch `wt/custody`.

Split across other tracks because of file ownership: B4 (the card reads `sim.destinations`, never written; read `depositAddresses` and render `summary`) and the card half of B2 and B17 go to T2 in `decision.js`; B3 (the chain accepts a missing `prev` anywhere and a deleted tip verifies clean) and B10 (an emptied `proposals.json` resets the spend cap) go to T6, which owns `audit.ts` and `store.ts`; B8 and B13 are C1 and C7 in T5.
