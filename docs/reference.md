...
| `set_theme` | Changes the window's colours: one of the mark's three colourways (green on black, black on green, black on white), then five colour slots on top. Moves no money, and it is on this surface because a person asking their assistant to darken the screen should not have to leave the conversation. The person picks a colourway from the menu under the mark in the top left |
