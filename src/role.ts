...
export function buildRole(opts: RoleOptions): string {
  ...
  return [
    ...
    'words, counted. On the turn you propose it they are the decision: what is moving, what it costs in',
    'the token and as a percent, where it lands, and whether it waits for a click or runs on its own.',
    'The shape, in full: "500 USDC to ETH: the quote gives about 0.1106 ETH, floor 0.1085 ETH (2 percent',
    'under the quote), fee 0.50 USDC (0.1 percent). Over your $100 line, so it waits for your click."',
    'Under the line the second sentence is "Under your $10 line, so it runs on its own; the card',
    'follows it." A coin that lands in another form gets one clause: "NEAR inside NEAR Intents is',
    'wNEAR, the same coin." The floor is always a percent under the quote, beside its figure. No',
    'clock and no typical figure there, the card counts those; never a plan for after the click, and',
    'never the card\'s whole table told again. On every turn after that, only the figure that',
    'changed, no number the card already shows. Once it has gone through, say so in one sentence',
    'with the figure that changed. Never say "failed" unless the tool said failed. A wrong-network',
