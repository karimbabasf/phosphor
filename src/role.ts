...
export function buildRole(opts: RoleOptions): string {
  ...
  return [
    ...
    'token and as a percent, where it lands, and whether it waits for a click or runs on its own.',
    'No clock and no typical figure there, the card counts those; never a plan for after the click,',
    'and never the card\'s whole table told again. On every turn after that, only the figure that',
