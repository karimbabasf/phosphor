...
const SWAP_SIDE = {
  ...
  toSymbol: z.string().max(128).describe('the coin bought: a symbol; an assetId only for a coin they named exactly'),
};
...
registerRead(
  'swap_quote',
  'What a swap would get right now, without filing anything: the amount in, the expected amount out, the minimum, the fee in dollars and the time it takes. Pass chain or toChain only when they named that network; otherwise the app picks each coin itself: the one in their balance to spend, the NEAR version to buy, so one coin never ends up spread across networks. Never ask which network; `picked` says which version was taken: say it in a few words. With no quote, or an amount over the balance, `sentence` says why in plain words: say that. `candidates` means the name fits several different coins: ask which coin, then quote by assetId. Use it before propose_swap whenever the coin or the size is new, so a probe never becomes a card. A coin they do not hold yet is priced too, as its NEAR version, with `preview`: the next step of a plan, never a swap to file. A plan in steps: quote every step first, each for what the one before gets; one with no price means file nothing.',
...
registerPropose(
  'propose_swap',
  'swap',
  `Proposes a swap inside their balance: one signed step that moves nothing on any chain. Pass chain or toChain only when they named that network: left out, the app picks each coin itself, the one in their balance to spend and the NEAR version to buy, so "swap my NEAR to USDC" needs neither and the balance never holds one coin on several networks. Never ask which network. A network is a coin's home (USDC on eth, not on arb), never a wallet. NEAR sits in the balance as wNEAR, the same coin. amountIn is "all" or the exact amount as text, never a rounded number. The app sets the minimum from its own live quote; pass minAmountOut only when they named one. Not sure a coin is listed, or what it gets? swap_assets and swap_quote answer without filing anything. If a name fits several different coins, the refusal lists them: ask which coin. It spends only what their balance holds: in a plan of steps, file the first once swap_quote priced every step, then ask them to say go when it lands: nothing wakes you then, so never promise the next. ${CANNOT_APPROVE}`,
  {
    ...
    toSymbol: z.string().max(128).describe('a symbol; an assetId only for a coin they named exactly'),
    amountIn: AMOUNT_IN,
