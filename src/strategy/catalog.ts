...
// The first line the agent reads, and the one that must never be stale. An agent that
// believes the money is pretend writes bolder programs than one that knows it is real.
const MONEY_IS_REAL =
  'Trading is on Hyperliquid. Every mandate you propose is armed against real money, and a rule that fires spends it. Size the envelope like it is the human real balance, because it is.';

const TRAPS = [
  MONEY_IS_REAL,
  EXIT_RULE,
  'propose_mandate ALWAYS waits for a human click. It grants standing authority rather than spending once, so there is no threshold that skips it. Expect to wait, and tell the human what you asked for in plain words.',
