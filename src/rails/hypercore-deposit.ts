...
//
//      The venue sets no floor on this route. 1Click's HyperCore leg credits through Circle CCTP
//      (a flat forwarding fee of about 0.32 USDC and no venue minimum); the "5 USDC or it is
//      lost" rule was the retired Bridge2's. The floor here is the fee's share: at 7 in the
//      nearly flat fee is about 4.8 percent, under the ceiling, and the draft's guaranteed floor
//      and the quote's guaranteed output are both held to 5 USDC delivered as a guard against a
//      fee far off its measured size.
//
...
// The least this app lets land, in USDC delivered. Not the venue's rule: the route credits
// through Circle CCTP, which has no minimum (the 5 USDC one was the retired Bridge2's). A guard:
// 7 in lands about 6.5, so this refuses only a quote whose fee is far off its measured size.
export const HYPERCORE_VENUE_MIN_CREDIT_USDC = 5;

// The size floor, in USDC sent. 7, for two reasons that both have to hold: at 7 the nearly
// flat fee is about 4.8 percent of the deposit, under the MAX_FEE_PCT ceiling below, and 7 in
// guarantees at least 5 lands after it (minCreditedFor(7) is 6.52). 6 was refused by the
// ceiling (5.5 percent, measured 2026-09-16), which is a floor that refuses everything at the
// floor. 1Click's own floor is lower.
export const MIN_DEPOSIT_USDC = 7;
// The least that may land, in HyperCore USDC base units (8 decimals), for the quote check,
// which compares base units to base units and never through a double.
...
export function hypercoreDepositRail(deps: HypercoreDepositDeps): HypercoreDepositRail {
  ...
  async function plan(draft: HlDepositDraft): Promise<{ plan?: Plan; reasons: string[] }> {
    ...
    if (draft.amountUsd < MIN_DEPOSIT_USDC) {
      return {
        reasons: [
          `${draft.amount} ${draft.symbol} is below the ${MIN_DEPOSIT_USDC} USDC floor. Deposits start at ${MIN_DEPOSIT_USDC} USDC because the ` +
            `routing fee is nearly flat (about ${HYPERCORE_MEASURED_FLAT_USDC} USDC) and would be over ${MAX_FEE_PCT} percent of anything smaller; ` +
            'deposit more at once',
        ],
      };
    }
    ...
    // The least that may land, against the draft's own guarantee, before any quote.
    if (draft.minCredited < HYPERCORE_VENUE_MIN_CREDIT_USDC) {
      return {
        reasons: [
          `${draft.amount} ${draft.symbol} would guarantee only ${draft.minCredited.toFixed(4)} USDC landing, under the ` +
            `${HYPERCORE_VENUE_MIN_CREDIT_USDC} USDC a deposit has to land; deposit more at once`,
        ],
      };
    }
    ...
    return {
      ...
      facts: {
        ...
        activity:
          ...
          `at least ${floor} USDC has to land, or nothing is signed.`,
        explorer: null,
      },
      lines: [
  ...
  function checkQuote(draft: HlDepositDraft, p: Plan, quote: OneClickQuote, feePct: number): string[] {
    ...
    // The least that may land first, then the draft's own floor. The route (Circle CCTP) has no
    // minimum of its own; this guards against a fee far off its measured size.
    if (guaranteed < VENUE_MIN_CREDIT_BASE) {
      problems.push(
        `the quote guarantees only ${formatUnits(guaranteed, HYPERCORE_USDC_DECIMALS)} USDC landing, under the ` +
          `${HYPERCORE_VENUE_MIN_CREDIT_USDC} USDC a deposit has to land. Deposit more at once`,
      );
      return problems;
    }
