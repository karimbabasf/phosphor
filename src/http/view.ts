...
/* Two kinds of tool live on this route and the difference is what they answer WITH.
   A chart write answers with the chart, so it hands its Outcome back and the shared tail below
   sends it. Everything else answers with its own thing (the theme it wrote, the board it posted
   to, the trading surface) and has already replied by the time it returns. */
type Handled = { done: true } | { done: false; outcome: Outcome };

const DONE: Handled = { done: true };
...
type ViewHandler = (a: ViewArgs) => Handled | Promise<Handled>;
...
function tradeWrite(apply: (a: ViewArgs) => Outcome): ViewHandler {
  return ({ ctx, args, res, ...rest }): Handled => {
...
const HANDLERS: Record<string, ViewHandler> = {
  ...
  profile_learned: ({ ctx, args, body, res }): Handled => {
  ...
  set_theme: ({ ctx, args, res }): Handled => {
  ...
  // ---------- the chart ----------
  chart_set_view: ({ ctx, args, res, by }): Handled => {
    // Resolve what was asked for into what a venue lists, before the view records it. Without
    // this the view stores the raw string, so "bitcoin" charts correctly and then labels itself
    // BITCOIN, and an agent reading the view back gets a product id no venue would recognise.
    ...
    const outcome = ctx.chart.setView(args, 'agent', by);
    // The chart store tidies its own levels, marks and trend lines on a product switch. The
    // drawing store is a separate file holding the same kind of object, so the sweep has to reach
    // it from here or half the agent's work would survive onto an instrument it does not
    // describe.
    ...
    return { done: false, outcome };
  },
  chart_add_indicator: ({ ctx, args, by }) => ({ done: false, outcome: ctx.chart.addIndicator(args, 'agent', by) }),
  chart_remove_indicator: ({ ctx, args }) => ({
    done: false,
    outcome: ctx.chart.removeIndicator(String(args.id ?? args.type ?? '')),
  }),
  ...
  chart_clear: ({ ctx, args, by }): Handled => {
    ...
    const outcome = ctx.chart.clear(what, by);
    // Same argument as the sweep in chart_set_view: a clear that left the zones behind would
    // leave the human looking at a chart the agent believes it cleaned.
    if (outcome.ok) {
      const removed =
        what === 'mine'
  ...
  chart_preset: ({ ctx, args, res, by }): Handled => {
    ...
    // cap, which is the outcome this whole path exists to prevent.
    ...
    for (const want of preset.indicators) {
      ...
      else notes.push(`${want.type} not added: ${added.error}`);
    }
    return { done: false, outcome: { ok: true, notes: [`preset ${preset.name}`, ...notes] } };
  },

  // ---------- the team ----------
  agent_post: ({ ctx, args, body, res }): Handled => {
  ...
  agent_spawn: ({ ctx, args, body, res }): Handled => {
...
export async function handleView(ctx: Ctx, body: JsonBody, res: http.ServerResponse): Promise<void> {
  ...
  const result = await handler({ ctx, args, body, res, by });
  ...
  ctx.sse.broadcastChart();
  // Answer with the chart as it now stands. An agent that has to call chart_read after every
  // write spends two round trips learning what its own change did.
  sendJson(res, 200, {
    ok: true,
    id: outcome.id,
    notes: outcome.notes,
