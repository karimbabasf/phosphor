...
/* The line the app appends to every message the human sends: which screen the window is on,
   ...
   The plan count is read off the trade service's `status` when it has one. The runner rebuild
   adds plans to that surface; a service without it cannot say, and the tag leaves the clause
   out rather than asserting zero. */
type TagSource = {
  view: { state(): { symbol: string } };
  status?: () => { plans?: { status?: string }[] } | undefined;
};
...
export function screenTag(view: string, trade: TagSource): string {
  ...
  const plans = trade.status?.()?.plans;
