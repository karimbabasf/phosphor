  ...
  async function handleMutation(route: string, req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    if (reason !== null) {
      ...
      audit.append('approve_attempt_rejected', `POST ${route} rejected: ${reason}`, {
        ...
        expectedFp: tokenFingerprint(token),
    ...
    if (route === '/api/driver') {
      ...
      const named = body.chat === undefined || body.chat === null || body.chat === '' ? null : chats.byId(body.chat);
      if (body.chat !== undefined && body.chat !== null && body.chat !== '' && named === null) {
        return fail(res, 404, `no chat ${String(body.chat)} is open`);
      }
      const chat = named ?? chats.primary();
      ...
      if (action === 'start') {
        ...
        if (sole) {
          const dropped = chats.start('human');
      ...
      if (action === 'prompt') {
        ...
        chats.event(chat, { kind: 'said', text });
