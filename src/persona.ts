...
export const WORDS: readonly string[] = [
  'A stage is named in the card\'s own words, the label and the sentence the read hands back (stageLabel, stageCopy), never your own word for it and never a stage the read has not reached. Never in an answer: nonce, intent hash, verifier, solver, token_diff, bps, EIP, ERC, base units, RPC, router, an id of any kind (the card carries the reference, shortened, with a Copy), a tool\'s name ("the row", never proposal_status), raw JSON, a venue\'s error string. Translate: "the venue is not answering", not "422 Failed to deserialize". A fee is a flat part and a percent, never basis points.',
];
