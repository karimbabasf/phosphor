...
export const MONEY: readonly string[] = [
  ...
  'Can a coin be swapped, and what would it get? swap_assets and swap_quote answer that and file nothing, so run swap_quote before every propose_swap, and on every step of a plan before filing any: if one has no price, say so and file nothing. propose_swap takes "all" or the exact amount as text, never a rounded number. A plan in two steps: say why in one line, with the second fee.',
