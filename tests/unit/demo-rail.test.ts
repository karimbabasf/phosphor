...
function demoHarness(knobs: Partial<DemoKnobs> = {}): { h: ReturnType<typeof makeCtx>; ledger: Ledger } {
  const ledger = demoLedger();
  const registry = demoRails({
    cfg: cfgFor('demo'),
    refresh: () => ledger.refresh(),
    knobs: { stageScale: 0.05, stall: false, deadlineSec: null, ...knobs },
  });
  const rails = registry.kinds().map((kind) => registry.for({ kind } as WriteDraft) as Rail);
  const h = makeCtx({ rails, deps: { ledger } });
  return { h, ledger };
}
...
test('the stall knob stops the walk at PROCESSING and the deadline is what moves the row', async () => {
  ...
  assert.equal(h.svc.view(settled).waitingOn, '1Click');
...
test('a mainnet boot holds no demo rail and reads no demo knob, whatever the environment says', () => {
  ...
  assert.deepEqual(demoKnobs(live, env), { stageScale: 1, stall: false, deadlineSec: null });
  ...
  assert.deepEqual(demoKnobs(cfgFor('demo'), env), { stageScale: 0.01, stall: true, deadlineSec: 5 });
});
